...
def check_split_parameters_consistency(chunk_size: int, stride: int, minimal_chunk_length: int) -> None:
    if chunk_size > 510:
        raise "Size of each chunk cannot be bigger than 510!"
    if minimal_chunk_length > chunk_size:
        raise "Minimal length cannot be bigger than size!"
    if stride > chunk_size:
        raise "Stride cannot be bigger than size! Chunks must overlap or be near each other!"
