...
def main() -> None:
    ...
    for num_experimento in tqdm(range(1,4), desc='Executando experimentos', colour='red'):
        for setModel in params['modelos']:
            ...
            classificador = ClassificaIndicios(batch_size=params['batch_size'], epochs=params['epochs'],
                            ...
                            modelo=setModel['modelo'], tokenizer=setModel['tokenizador'], train=params['train'], test=params['test'], val=params['val'], coluna=params['coluna'])
        
            print("Treinando modelo...")
            metricas = classificador.modelTraining()
            print("Salvando resultados do modelo...")
            writeJson(os.path.join(params['dir_save_metrics'], 'metrics_indicios-{}-{}.json'.format(num_experimento, setModel['model_name'])), metricas)
