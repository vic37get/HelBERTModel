from crossValidObjeto import ClassificaObjetosTipo
import sys
sys.path.insert(0, '../../')
...
def main() -> None:
    ...
    for num_experimento in tqdm(range(1,6), desc='Executando experimentos', colour='red'):
        for setModel in params['modelos']:
            print("Classificando indicios com o modelo: " + setModel['model_name'])
            classificador = ClassificaObjetosTipo(batch_size=params['batch_size'], epochs=params['epochs'],
            ...
            writeJson(os.path.join(params['dir_save_metrics'], 'metricsObjeto-{}-{}.json'.format(num_experimento, setModel['model_name'])), metricas)
