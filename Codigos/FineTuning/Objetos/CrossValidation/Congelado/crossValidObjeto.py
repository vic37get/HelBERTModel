...
class ClassificaObjetosTipo:
    ...
    def calculate_mean_global_metrics(self, metricas: list) -> dict:
        ...
        for key in metricas[0].keys():
            if key == 'accuracy':
                ...
                mean_metrics[key] = {'precision': precision, 'recall': recall, 'f1-score': f1_score,
                                      'support': support}
        return mean_metrics
    ...
    def val_step(self, val_loader: DataLoader, epoch: int, fold: int):
        ...
        return {'fold': fold, 'epoca': epoch, 'val_losses': mean(val_losses), 'accuracy': acuracia, 'cf_report': cf_report, 'hloss': hl}
    ...
    def modelTraining(self, folds: list, train_dataset: MyDataset) -> None:
        ...
        """
        history={'train_losses': [],'val_losses': [], 'cf_report': [], 'f1_curve': [], 'train_loss_curve': [],
                  'val_loss_curve': [], 'hloss': [], 'time_per_epoch': [], 'epochs_per_fold': []}
